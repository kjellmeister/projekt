...
class CRequest
{
  ...
  public function Init($baseUrl = null, $routing=null) 
  {
	    ...
	    $this->baseUrl       = !empty($this->baseUrl) ? $this->baseUrl : "{$parts['scheme']}://{$parts['host']}" . (isset($parts['port']) ? ":{$parts['port']}" : '') . rtrim(dirname($scriptName), '/');
	    
	    
	    
	    // Store it
	    $this->base_url      = rtrim($this->baseUrl, '/') . '/';
